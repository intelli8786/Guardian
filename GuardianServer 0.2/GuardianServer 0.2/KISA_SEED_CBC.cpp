...
#define EndianChange(dwS)								\
	((ROTL((dwS), 8) & (SDWORD)0x00ff00ff) | \
	(ROTL((dwS), 24) & (SDWORD)0xff00ff00))				\
...
{
	SDWORD L0, L1, R0, R1;		// Iuput/output values at each rounds
	SDWORD T0, T1;				// Temporary variables for round function F
	SDWORD *K = ks->key_data;	// Pointer of round keys
...
SDWORD* chartoint32_for_SEED_CBC(IN BYTE *in, IN int nLen)
{
	unsigned int *data;
	int len, i;
...
BYTE* int32tochar_for_SEED_CBC(IN SDWORD *in, IN int nLen)
{
	unsigned char *data;
	int i;
...
{
	SDWORD A, B, C, D;							// Iuput/output values at each rounds
	SDWORD T0, T1;								// Temporary variable
	SDWORD *K;
	...
	A = ((SDWORD *)pbszUserKey)[0];
	B = ((SDWORD *)pbszUserKey)[1];
	C = ((SDWORD *)pbszUserKey)[2];
	D = ((SDWORD *)pbszUserKey)[3];
...
int SEED_CBC_Process(OUT KISA_SEED_INFO *pInfo, IN SDWORD *in, IN int inLen, OUT SDWORD *out, OUT int *outLen)
{
...
int SEED_CBC_Close(OUT KISA_SEED_INFO *pInfo, IN SDWORD *out, IN int *outLen)
{
...
void main()
{
	...
	int nPlainTextLen = sizeof(plainText);
	int nCipherTextLen = 0;

	printf("Key        : ");
	for (int i = 0; i<16; i++)	{ printf("%02X ", pbszUserKey[i]); }
	printf("\nIV       : ");
	for (int i = 0; i<16; i++)	{ printf("%02X ", pbszIV[i]); }
	printf("\nPlaintext(%d)  : ", nPlainTextLen);
	for (int i = 0; i<nPlainTextLen; i++)	{ printf("%02X ", plainText[i]); }

	printf("\n\n");

	nCipherTextLen = SEED_CBC_Encrypt(pbszUserKey, pbszIV, plainText, nPlainTextLen, pbszCipherText);	//¾ÏÈ£È­
	...
	printf("\nPlaintext(%d)  : ", nPlainTextLen);
	for (int i = 0; i<nPlainTextLen; i++)
	{
		printf("%02X ", pbszPlainText[i]);
	}
}